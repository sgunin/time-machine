...
/*----------------------------------------------------------------------------------------------
 * Function					: GenerateBaseCarrier(boost::circular_buffer<double>& carrArrayI,
							 boost::circular_buffer<double>& carrArrayMI)
* Abstract					: This function generates the base carrier i.e cosine waveform for the modulation 
*							  of the signal from the satellite. Since its BPSK, then for the bit duration and sign of the bit
*							  the symbol (cosine wave) is used. for bit = 1, +ve cosine wave is the analog signal and for bit = 0 or -1 
*							  -ve cosine wave is the analog signal used. Hence in this function we generate a base cycle for -ve and +ve cosine
*							  wave signal. 
 * Formal
 * Parameter(s) 			: refernence to the array
 * Return value 			: +ve and -ve cosine array of size = (carrier resolution), carrArrayI, carrArrayMI)
 * System Call				: None
 * Functions called			: None
 * Reference				: None
 * Specific library calls	: Boost Circular buffer ( boost 1.42)
 * Member variables accessed: None
 * Assumptions 				: None
 *
 ----------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------
 * Function					: string GetGeneratedCarr(modulator_opDataPort& generatedCarr, 
								boost::circular_buffer<double> & carrArrayI,
								double& initCoarsePhase,
								double& initCarrierFinePhase,
								double& satCarrClk)
 * Abstract					: This function generate a structured buffer containing no. of completed cycle,
							  residual coarse phase and residual fine phase, depending on the initial phase
							  and freqency of the clock given
 *							  given the satellite taps.
 * Formal
 * Parameter(s) 			: initial phase, coarse phase, and the frequency to be generated
 * Return value 			: generated Carrier buffer with the number of completed cycles and residual
 * System Call				: None
 * Functions called			: None
 * Reference				: None
 * Specific library calls	: boost circular buffer (boost 1.42)
...
// find the total number of completed cycle i.e integer part of the frequency
double totalCompleteCycles = std::floor(tempCarrCycles);

// fractional part of the completed cycle will account for coarse and fine phase
double fractionalCycles = tempCarrCycles - totalCompleteCycles;

// get the integer number of coarse phase points by mulitplying with carr cycle resolution 
// More the cycle resolution better would be the estimate. here its assumed to be 16
double totalPhaseInFracCycle = fractionalCycles * CARR_CYCLCE_RESOLUTION; // depends points make a cycle

// Find the fine remaining phase of the carrier cycle, this is less than 1 carrier resolution point
double integerPhase = std::floor(totalPhaseInFracCycle);

// this gives the fine carrier phase with an accuracy < 1 carrier resolution point, lesser this vallue better is our estimation
double fracphase = totalPhaseInFracCycle - integerPhase;

// Now add initial phase and intial coarse phase (number of carrier resolution points) and resolve for 
...
// this will give the first circular buffer that will give the starting point of the carrier wave
...
// resoulve for the last part of the carrier wave
// apart from the first (starting phase of carrier wave) and the last
// end cycle of the carrier wave is only effected by phase noise and other randomm event
// any change greater than 2*pi would be turned into a completed cycle and we need to see the residual cycle only
...
for(phaseIndex = tempArray.begin();phaseIndex !=phaseIndexLast;)
 {
	// generatedCarr.lastBlockIF.push_back(*phaseIndex);
	  generatedCarr.firstBlockIF.push_back(*phaseIndex);
	 content += leftJustify(asString((double)*phaseIndex), 5);
	 content += leftJustify("\n",3);
	 phaseIndex++;
