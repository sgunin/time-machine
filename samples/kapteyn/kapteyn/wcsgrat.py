...
def makelabel(hmsdms, Hlab, Dlab, Mlab, Slab, prec, fmt, tex):
   ...
   *Slab* are all set to True.
   ...
   :param hmsdms:
      The output of function :func:`gethmsdms`
   :type hmsdms:
...
class Insidelabels(object):
   ...
   def setp_label(self, position=None,
      ...
      :param fmt:
            ...
            See also the examples at :func:`makelabel`.
      :type fmt: String
...
def getStarts(startvals, proj, i, axnum, wcstype, mixpix=None):
   ...
   if not isinstance(startvals, str):
...
.. automethod::   get_aspectratio
   ...
   def __init__(self, header=None, graticuledata=None, axnum=None,
      ...
      if not deltax is None and isinstance(deltax, str):
      ...
      if not deltay is None and isinstance(deltay, str):
         # Not empty and the type is a string. Then a unit can follow.
         parts = deltay.split()
         if len(parts) > 1:
            uf, errmes = unitfactor(parts[1], axisunits)
      ...
      kwargs1 = {'fontsize':10}
      kwargs2 = {'fontsize':10}
      kwargs3 = {'fontsize':10, 'rotation':270, 'visible':False}
      kwargs4 = {'fontsize':10, 'visible':False}
   ...
   def plot(self, framebase, frame=None, frame2=None):
      ...
      Notes:        In Sep 2012 we changed this method to be able to
                    ...
                    29 Jul 2019, VOG: Changed routines be able to
                                      set ticks inwards or outwards
                                      with keyword 'direction='
      """                    
      #-----------------------------------------------------------
      ...
      if not frame:
         ...
         try:
            # VOG, 06-07-2023: Deprecated: r,c,n = framebase.get_geometry()
            r,c,n = framebase.get_subplotspec().get_topmost_subplotspec().get_geometry()[:3]
            n += 1
      ...
      for tick, kw, mkw in zip(frame.yaxis.get_major_ticks(), kwargs[left], Mkwargs[left]):
         if len(kw) != 0:
             pad = kw.get("pad", None)
             if pad:
                fkw = {i:kw[i] for i in kw if i!='pad'}  # Filter pad key, preserve kw
                tick.set_pad(pad)
                tick.label1.set(**fkw)
             else:
                tick.label1.set(**kw)
         direction = 'in'
         if len(mkw) != 0:
            direction = mkw.get("direction", None)
            if direction:
                fmkw = {i:mkw[i] for i in mkw if i!='direction'}  # Filter direction key, preserve mkw
                tick.tick1line.set(**fmkw)
            else:
                tick.tick1line.set(**mkw)
         # VOG, 06-07-2023: tick.apply_tickdir(direction)
         frame.yaxis.set_tick_params(direction=direction)
         tick.tick2on = False
         tick.tick2line.set_visible(False)
         # VOG 06-07-2023 not allowed anymore: tick.tick1line.set_marker(tick._tickmarkers[0])  # Hope this attribute will remain in newer releases
      ...
      for tick, kw, mkw in zip(frame.xaxis.get_major_ticks(), kwargs[bottom], Mkwargs[bottom]):
         ...
         if len(mkw) != 0:
            ...
            if direction:
                ...
                tick.tick1line.set(**mkw)
         # tick.apply_tickdir(direction) 
         frame.xaxis.set_tick_params(direction=direction)
         tick.tick2on = False
         # VOG 06-07-2023, tick.tick1line.set_marker(tick._tickmarkers[0])
         #for line in (tick.tick1line,):    #: VOG Turned out not to be necessary
         #    line.set_markersize(tick.tick1line.size)
         tick.tick2line.set_visible(False)
      ...
      for tick, kw, mkw in zip(frame2.xaxis.get_major_ticks(),kwargs[top], Mkwargs[top]):
         ...
         if len(mkw) != 0:
            ...
            if direction:
                fmkw = {i:mkw[i] for i in mkw if i!='direction'}  # Filter direction key, preserve mkw
                tick.tick2line.set(**fmkw)
            else:
                tick.tick2line.set(**mkw)
         # tick.apply_tickdir(direction) 
         frame2.xaxis.set_tick_params(direction=direction)
         # VOG 06-07-2023, tick.tick2line.set_marker(tick._tickmarkers[1])
   ...
   def setp_tick(self, wcsaxis=None, plotaxis=None, position=None,
      ...
      :param markerdict:
            Properties for the tick marker. Amongst others:
               ...
               * markersize:
                 Size of tick line. 
               * direction:
                 Use keyword argument 'direction='
                 get tick lines that point outside the plot instead
                 of the default which is inside.
               * Set the tick label pad in points. Use a negative number to
                 shift a label inwards.
               * markeredgewidth:
      ...
      :note:
            ...
            To set the direction of the tickmarks, use keyword argument
            *direction=* with one of the options 'in' (default),
            'out' or 'inout'. In the set with example code, one can find
            examples with this keyword.
      ...
      :Examples:
         ...
         3. Using the utility routines :meth:`setp_ticklabel()` 
         and :meth:`setp_tickmark()`::
   ...
   def setp_tickmark(self, wcsaxis=None, plotaxis=None, position=None,
      ...
      and *markersize*. Keyword direction= sets a tick direction to
      inwards or outwards, or both ('in', 'out', 'inout').

      See also documentation for :func`wcsgrat.setp_ticklabel`.
      """
      #-----------------------------------------------------------------
...
# VOG: from  matplotlib.mathtext import Char
# VOG: Char._update_metrics = _update_metrics
