...
#          Aug 03,       2019; v1.30. Major migration to Python 3. Adjustments for Matplotlib 3. (3.1.1)
#                              and improvements for tick labeling (direction and pad) and other
#                              improvements. Uses axes divider to keep height/width of color bar consistent
#                              with height/weight other Axes objects after resizing.
#          Nov 22,       2019; Crosshair cursor extends to panels and added shift arrow 
#                              to move cursor 1 data pixel.
#          Jun 07,       2021; Added remove() method to graphical objects line beam, skypolygon etc.
#                              Added pixel option for skypolygon.
#          Jun 28,       2022; Updated ffmpeg parameters. Removed bitrate. Added frame rate for input, separated
#                              from frame rate of output video. Solved problem with splitscreen(). Data needed
#                              to be set with setter. Added remove function for Pixellabels object
#
#__version__ = 'x.xx'  Do not define it here because the first line in this
...
* Plot world coordinate labels along the image axes  (basic routines in :mod:`kapteyn.wcsgrat`)
* Plot coordinate graticules (basic routines in :mod:`kapteyn.wcsgrat`)
...
as with the building blocks in this `maputils` module.

Module `maputils` is also very useful as a tool to extract and plot
...
# From some Matplotlib version on, the default direction for tick marks
# changed from inwards to outwards. Here we restore the behaviour like
# it was for pre Python 3 versions with older Matplotlibs
# We changed also method setp_tickmarks() in wcsgrat so that it mimics the 
# Matplotlib method Axes.tick_params()
try:
...
# VOG: from kapteyn.interpolation import map_coordinates  # original from scipy.ndimage.interpolation
from scipy.ndimage import map_coordinates
# VOG: from kapteyn.filters import gaussian_filter
...
# VOG, 05-07-2023 The KeyPressFilter does not work anymore in MPL 3.7
# But we can filter in another way, using rcParams settings.
# I chose to disable all MPL key shortcuts in the canvas. They are all
# available in the navigation toolbar.
# The full screen option is kind of destructive so needed to be filtered anyhow.
...
#
# These effects are realized by redefining the callback functions and adding the 
...
def removeLinesandTextfromframe(ax):
    # One cannot remove plot elements from a frame anymore with setting
    # its attribute to an empty list e.g. ... .lines = []
...
# For other os, use cbook.reports_memory() to get the memory pages.
...
#-------------------------------------------------------------------------------
   ...
   if system.lower().startswith('linux'):
      command = ['ps', '-p', '%d' % pid, '-o', 'sz,rss']
      linux = True
   elif system.lower().startswith('darwin'):
      command = ['ps', '-p', '%d' % pid, '-o', 'vsz,rss']
      darwin = True      
   if linux or darwin:
      ou = check_output(command)
      ou1 = ou.split()                      # split bytes objects in list of 4
      mem = int(ou1[-1].decode("utf-8"))    # Convert last value (rss in kB) to a string type
      #p = Popen(command , shell=True, stdout=PIPE)
      #a = p.stdout.readlines()
      #mem = int(ou[1].split()[1])                     # Res. memory in kB
      #mem = int(a[1].split()[1])                     # Res. memory in kB
      mem /= 1024                                    # in mB
      mem = "Resident mem.: " + str(int(mem)) + " MB "
      #p.stdout.close()
      #p.kill()
   else:
      try:
...
def showall():
   ...
   and its derived objects, with method :meth:`Annotatedimage.plot`.
   ...
   for name in list(kwargs.keys()):
     ...
     if value is None:
        ...
        if dicttype:
           ...
           hdr[fitsname] = value   # VOG the same as from astropy 0.1
...
class Contours(object):
   ...
   def plot(self, frame):
      ...
      if self.clevels is None:
         if self.filled:
            self.CS = self.frame.contourf(self.data, cmap=self.cmap, norm=self.norm, **self.kwargs)
         else:
         ...
         if self.filled:
            self.CS = self.frame.contourf(self.data, self.clevels, cmap=self.cmap, norm=self.norm, **self.kwargs)
         else:
   ...
   def setp_contour(self, levels=None, **kwargs):
      ...
      :Examples:
         ...
         >>> cont = annim.Contours(levels=list(range(10000,16000,1000)))
   ...
   def remove(self):
      #--------------------------------------------------------------------
      # Remove matplotlib object after which it will not be redrawn
      #--------------------------------------------------------------------         
...
class Colorbar(object):
   ...
   """
   #--------------------------------------------------------------------
   def __init__(self, cmap, location=None, size="5%", pad=0.05,
                frame=None, norm=None, 
                contourset=None, clines=False, fontsize=9,
                label=None, linewidths=None, visible=True, **kwargs):
      #--------------------------------------------------------------------
      """
      cmap: Applies only for Matplotlib versions <= 0.99.8. 
            This and some other arguments are 
            supplied by the :meth:`Annotatedimage.Colorbar` method.            
      location: The location of the colorbar can be one of 'left', 'bottom', 
            'top' or 'right'.
            The keyword argument 'orientation' should be in agreement with
            the location. The arguments 'location', 'size' and 'pad' are
            the three arguments for method divider.append_axes().
      size: Size of colorbar in percentage og mother axes object (e.g. image).
      pad:  Pad between image axes and colorbar axes
      ...
      if not location:
         if not orientation:
            ...
            if orientation == 'horizontal':
               ...
               location = 'right'
      self.location = location   # We use the axes divider from the MPL toolkit
   ...
   def plot(self, cbframe, mappable=None):
      ...
      if self.plotcontourlines and self.contourset is not None:
         ...
         # VOG, 07-07-2023: The add_lines() method complains. The object should
         # be a ContourSet, but it is a QuaContourSet and then MPL's colorbar class 
         # throws an exception. So we try something else:
         """
         ...
         if len(tcolors) != len(CS.levels):
             # For filled contours there is 1 color and 1 linewidth less.
             tcolors.append((0,0,0,0))
         lw = self.linewidths
         if not lw:
            lw = [1.5]*len(CS.levels)
         if numpy.isscalar(lw):
            lw = [lw]*len(CS.levels)
         if len(lw) != len(CS.levels):
             lw.append(lw[-1])
         self.cb.add_lines(CS.levels, tcolors, lw)
      else:
          CS = None
...
class Beam(object):
   ...
   def plot(self, frame):
      #--------------------------------------------------------------------
      """
      Plot a beam with the parameters given in the constructor.
      """
      #--------------------------------------------------------------------
...
#--------------------------------------------------------------------
   def __init__(self, projection, prescription=None,
      ...
      if len(lon1):
         if grids:     # Here we have only lon1, not lon2
             xp, yp = lon1, lat1
         else:
             xp, yp = projection.topixel((lon1, lat1))
         s = str(xp)
...
#--------------------------------------------------------------------
   """
   Gridplot class. Usually the parameters will be provided by method
   :meth:`Annotatedimage.Marker`

   Use Matplotlib's plot function to plot lines etc. in the current
   grid system. Properties of the
   marker are set with Matplotlib's keyword arguments.
   """
#--------------------------------------------------------------------
   def __init__(self, gridsX, gridsY, **kwargs):
      self.ptype = "Gridplot"
      self.xp = gridsX
      self.yp = gridsY
      self.kwargs = kwargs
   ...
   def plot(self, frame):
      self.lines = frame.plot(self.xp, self.yp, **self.kwargs)[0]   # First returned 
...
#--------------------------------------------------------------------
   ...
   def plot(self, frame):
      ...
      if not issequence(self.yp):
         self.yp = [self.yp]
      # VOG: default 'o' not necessary: self.patch = frame.plot(self.xp, self.yp, 'o', **self.kwargs)  # Set default marker symbol to prevent connections
      self.patch = frame.plot(self.xp, self.yp, **self.kwargs)  # Set default marker symbol to prevent connections
...
class Pixellabels(object):
   ...
   :param direction:    Set direction of tick marks to inwards or outwards.
                        There are three options: 'in', 'out', 'inout'
   :type  direction:    String

   :param pad:          Set a value for the distance between axis and tick
                        mark label. By default, this value is set to None.
                        If you did set 'inside=True', the default padding
                        will be -10,but this value can be overruled by
                        a value for 'pad'. Note that padding is applied to
                        all axes.
   :type pad:           None or Float
   ...
   :param inside:       If True, plot labels within the plot, else outside
   ...
   :Examples:           Annotate the pixels in a plot along the right and top axis
                        ...
                        Example using the functions that set the properties of the tick markers
                        and tick labels::
   ...
   .. automethod:: setp_label
   """
   def __init__(self, pxlim, pylim, plotaxis=None, direction='in', pad=None,
   ...
   def setp_marker(self, **kwargs):
      #--------------------------------------------------------------------
      """
      Set properties of the pixel label tick markers for all axes. For 
      ticks in graticules we allowed keyword *direction=*, but that was 
      to facilitate setting a direction for all axes seperately.
      For pixel labels, we apply the tick properties to all axes, and
      because the tick marker direction is not a property of a Line2D 
      object, we allow setting the tick mark direction only in the 
   ...
   def setp_label(self, **kwargs):
      #--------------------------------------------------------------------
      """
      Set properties of the pixel label tick markers for all axes
   ...
   def plot(self, frame):
      ...
      # If labels must be plotted inside the frame, the 
      # value of inside= is set to True.
      # Then we apply a pad and an alignment. But setting
      # padding with tick.set_pad() overwrites all
      # properties of tick's Line2D and Text objects.
      # So this must be done beforehand.

      setmarker = len(markerkwargs) > 0
      labinside = self.gridlabs.inside
      if self.pad:
         padding = True
         pad = self.pad
      else:
      ...
      for tick in gframe.xaxis.get_major_ticks():
         if padding:
            tick.set_pad(pad)
         #tick.apply_tickdir(self.direction)
         gframe.xaxis.set_tick_params(direction=self.direction)
         if 3 in plotaxes:
            ...
            #tick.tick2line.set_marker(tick._tickmarkers[1])
         elif 1 in plotaxes:
            ...
            tick.tick2On = False
            tick.gridOn = pixellabels.gridlines
            #tick.tick1line.set_marker(tick._tickmarkers[0])
      ...
      for tick in gframe.yaxis.get_major_ticks():
         if padding:
            tick.set_pad(pad)
         #tick.apply_tickdir(self.direction)
         gframe.yaxis.set_tick_params(direction=self.direction)
         if 2 in plotaxes:
            tick.label2.set(**kwargs)
            if setmarker:
               tick.tick2line.set(**markerkwargs)
   ...
   def remove(self):
      ...
      # for minor ticks
      self.frame.set_xticks([], minor=True)
      self.frame.set_yticks([])
      # for minor ticks
      self.frame.set_yticks([], minor=True)
...
:param projection:
   ...
   :meth:`kapteyn.wcs.Projection.toworld` and :meth:`kapteyn.wcs.Projection.topixel`
...
:param gridmode:
   ...
   with method :meth:`kapteyn.positions.str2pos`. These are not the
...
:Attributes:
    ...
    .. attribute:: objlist
          ...
          'Pixellabels', 'RGBimage', 'Ruler', 'Skypolygon', 'Gridplot'
...
#--------------------------------------------------------------------
   ...
   def __init__(self, frame, header, pxlim, pylim, imdata, projection, axperm, wcstypes,
      ...
      self.alpha = 1.0
      self.resize_id = self.frame.figure.canvas.mpl_connect('resize_event', self.onresize)
      self.divider = None   # Gets a value when Axes divider is used for a colorbar
      

   def onresize(self, event):
      #-----------------------------------------------------------------
      """
      Helper callback function to deal with a resize event defined 
      in __init__() above. This call is necessary because frames with
      the same Bbox do not scale the same when a figure is resized.
      Newer versions of Matplotlib (3.1.x) seem to deal with space
      for axes labeling and an Axes object with labels scales different 
      than the Axes object without. This resize callback forces the 
      Bbox to remain the same.
      """
      #-----------------------------------------------------------------
      if self.divider:
         p1 = self.divider.locate(0,0)
      else:
         p1 = self.frame.get_position()
      ...
      for obj in self.objlist:
         ...
         if pt == 'Graticule':
             obj.frame.set_position(p1)
             obj.frame2.set_position(p1)
         if pt == 'Pixellabels':
             obj.frame.set_position(p1)
   ...
   def histeq(self, nbr_bins=256):
      ...
      #get image histogram
      imhist,bins = numpy.histogram(im[ix].flatten(), nbr_bins, density=True,
   ...
   def Contours(self, levels=None, cmap=None, colors=None, filled=False, **kwargs):
      #-----------------------------------------------------------------
      """
      ...
      :param filled:
         if set to True the area between two contours will
      ...
      contourset = Contours(self.data, self.box, levels, cmap=cmap, norm=self.norm, colors=colors, filled=filled, **kwargs)
   ...
   def Colorbar(self, frame=None, clines=False, **kwargs):
      ...
      :param kwargs:
          ...
          * *location* - The location of the colorbar can be one of 'left', 'bottom', 
            ...
            The keyword argument 'orientation' should be in agreement with
            the location. The arguments 'location', 'size' and 'pad' are
            the three arguments for method 
            :meth:`mpl_toolkits.axes_grid1.make_axes_locatable.divider.append_axes()`.
          * *siz*: Size of colorbar in percentage og mother axes object (e.g. image).
          * *pad*  Pad between image axes and colorbar axes
      ...
      :Examples:
          ...
          >>> mplim.Image()
          >>> units = r'$ergs/(sec.cm^2)$'
          >>> colbar = mplim.Colorbar(location='left', pad=0, size="2%", fontsize=8)
          >>> colbar.set_label(label=units, fontsize=10, rotation=-90, labelpad=-40)
          >>> mplim.plot()
          >>> plt.show()
   ...
   def Graticule(self, visible=True, pxlim=None, pylim=None, **kwargs):
      ...
      :Methods:
         ...
         * :meth:`kapteyn.wcsgrat.Graticule.Ruler`
         * :meth:`kapteyn.wcsgrat.Graticule.Insidelabels`
   ...
   def Minortickmarks(self, graticule, partsx=10, partsy=10, **kwargs):
      ...
      :Returns:
         ...
         (e.g. :meth:`kapteyn.wcsgrat.Graticule.setp_tickmark`).
   ...
   def Beam(self, major, minor, pa=0.0, pos=None, xc=None, yc=None, units=None, **kwargs):
      ...
      if pos is not None:           
         ...
         if grids:
            world = poswp[1][0]      # Only first element of the parsed position is used
         else:
            world = poswp[0][0]
      else:
         world = (xc, yc)
   ...
   #-----------------------------------------------------------------
      ...
      *lons* and *lats*.  These are copied unaltered as vertices for
      an irregular polygon. The coordinates are expected to be grids
      (e.g. for a GIPSY set) and are converted to pixels first.
      ...
      :Examples:
       ...
       ::
          ...
          annim.skypolygon('ellipse', cpos='10 20',major=3.50, minor=1.50, 
                            units='arcmin',fc='g', alpha=0.3, lw=3, ec='r')",
          annim.skypolygon('rectangle', cpos='-51.1898431 deg 60.1476405 deg', major=3.50, minor=1.50, 
                            units='arcmin',fc='g', pa=-20, alpha=0.3, lw=10, ec='r')",
          annim.skypolygon(lons=[10,20,30], lats=[2,4,3], units='PIX')                            
   ...
   #-----------------------------------------------------------------
      ...
      if prescription is None and units.upper().startswith('PIX'):
         # User/programmer wants a polygon in grids (so we need to convert
         # the grids to pixels)
         lons, lats = self.projection.grid2pixel((lons,lats))             
   ...
   def Marker(self, pos=None, x=None, y=None, mode='', **kwargs):
      ...
      :Examples:
         ...
         coordinates::
            ...
            pos = "[200+20*sin(x/20) for x in range(100,200)], list(range(100,200))"
   ...
   def Gridplot(self, gridsX, gridsY, **kwargs):
      #-----------------------------------------------------------------
      """
      Use Matplotlib's 'plot' function for the curent grid coordinate system.
      All parameters are grid coordinates and depending on the header values
      of CRPIX, they are converted to pixel coordinates (1..N) and then plotted
      by Matplotlib's plot() function.

      :param gridsX:
         Sequence of X values in grids         
      :type gridsX:
         Sequence of numbers, similar to what you enter in 
         Matplotlib's plot() function
      :param gridsY:
         Sequence of Y values in grids         
      :type gridsY:
         Sequence of numbers, similar to what you enter in 
         Matplotlib's plot() function
      :param kwargs
         All keyword arguments are transferred to Matplotlib's
         plot() function. Examples are found in e.g.:
         https://matplotlib.org/stable/api/_as_gen/matplotlib.lines.Line2D.html#matplotlib.lines.Line2D
         (alpha, color, linestype etc.)
      :type kwargs:
      ...
      :Returns:
         ...
         Object from class :class:`Gridplot`
      ...
      :Examples:
         ...
         ::
            ...
            annim = f.Annotatedimage(frame, cmap="binary")
            annim.Image()
            annim.Gridplot([0,10,5], [3,4,6], color='r')
         
      """
      #-----------------------------------------------------------------
      # The coordinates enter as grids and convert to pixels in function 
      # gridplot
      xp, yp = self.projection.grid2pixel((gridsX,gridsY))
      lines = Gridplot(xp, yp, **kwargs)
      self.objlist.append(lines)
      return lines
   ...
   def Ruler(self, pos1=None, pos2=None,
      ...
      :func:`kapteyn.rulers.Ruler` in module :mod:`rulers`
   ...
   def plot(self):
      ...
      Plotting in this context implies that we call the *plot()* methods
      of all the objects found in this Annotatedimage object. The methods
      require an Axes object. That is usually trivial but Colorbar objects 
      derive their Axes from a 'mother' Axes object. We use the AxesDivider
      technique to make the new Axes and Colorbar Axes relocatable.
      This keeps height and width of the Colorbar Axes in sync with the 
      mother Axes. We copy the Bbox properties from the new Axes to the
      other existing Axes objects in a resize callback. This callback is 
         ...
         try:
            ...
            if pt == "Colorbar":
               if obj.frame is None:             # No frame set by user
                  self.divider = make_axes_locatable(self.frame)
                  self.cbframe = self.divider.append_axes(obj.location, size=obj.size, pad=obj.pad)
               else:
                  self.cbframe = obj.frame
         except:
            raise Exception("Unknown object. Cannot plot this!")
      
         # Before locatable Axes, we used: self.cbframe = make_axes(self.frame, orientation=orientation)[0]
      ...
      for obj in self.objlist:
         ...
         if pt in ["Image", "Contour", "Graticule", "Insidelabels", "Pixellabels", "Beam",
                   "Marker", "Ruler", "Skypolygon", "Gridplot"]:
            try:
               visible = obj.visible
            except:
               visible = True
      ...
      # A colorbar is created. It gets its correct Axes after a resize event.
      # But if we save to a figure (e.g. te create plots in Sphinx), there is no
      # resize event, so for this situation we force one. (VOG 06-08-2019)
      #if self.divider:
      self.onresize(None)
   ...
   def toworld(self, xp, yp, matchspatial=False):
      #--------------------------------------------------------------------
      """
      This is a helper method for method :meth:`kapteyn.wcs.Projection.toworld`.
   ...
   def topixel(self, xw, yw, matchspatial=False):
      #--------------------------------------------------------------------
      """
      This is a helper method for method :meth:`kapteyn.wcs.Projection.topixel`.
   ...
   def inside(self, x=None, y=None, pos=None, mode=''):
      ...
      :param y:
         Single number or a sequence representing the x coordinates
      ...
      :Note:
         ...
         with respect to the use of method :meth:`kapteyn.positions.str2pos`.
      ...
      :Examples:
         ...
         >>> x = list(range(180,400,40))
         >>> y = list(range(100,330,40))
      ...
      if not (pos is None):
         ...
         if world:
            # Allow world coordinates that cannot be converted to pixels
            # Nan's will be returned for those world coordinates.
            # This setting uses the allow_invalid flag.
            # Note that topixel() can still go wrong somehow.
            # In that case x and y will be set to None by topixel() 
            # (method of Projection object)
            ainv = self.projection.allow_invalid
            self.projection.allow_invalid = True
            xp, yp = self.topixel(x, y)
            self.projection.allow_invalid = ainv
            # When conversions fail then None's are returned
            if not issequence(x):
               x = [x]
            if not issequence(y):
               y = [y]
            if xp is None:
               xp = numpy.array([numpy.nan]*len(x))
            if yp is None:
               yp = numpy.array([numpy.nan]*len(y))
         else:
   ...
   def disconnectCallbacks(self):
      ...
      self.frame.figure.canvas.mpl_disconnect(self.resize_id)
   ...
   def mouse_writepos(self, axesevent):
      ...
      """
      #--------------------------------------------------------------------
      condition = axesevent.event.button in [1,2,3] and axesevent.event.key == 'shift'
      if not condition:
         return
      ...
      if self.figmanager.toolbar.mode == '':
         ...
         if axesevent.event.button == 2:
            # Added at June 9, 2021: With the shift + middle mouse button we 
            # send a string 'DEL' to the command line. This can be used 
            # to take an action in the current active task that recieves the 
            # positions on the command line. An example can be found in ELLFIT
            s = 'DEL'

         if axesevent.event.button == 3:
            # Added at June 9, 2021: With the shift + right mouse button we 
            # send an carriage return only. An external program can react to this
            # e.g. in ELLFIT to close a polygon.
            s = ''
   ...
   def positionsfromfile(self, filename, comment, skyout=None, **kwargs):
      ...
      :meth:`kapteyn.tabarray.readColumns()`
...
#--------------------------------------------------------------------
   ...
   #------------------------------------------------------------
      ...
      if isinstance(self.hdr, dict):
         ...
         for s in self.hdr.cards:          #VOG: previously ascardlist():
            if not str(s).startswith('HISTORY'):
               st += "%s\n" % s
      return st
   ...
   def getaxnumberbyname(self, axname):
      ...
      for i in range(n):
         ...
         if str2.find(axname.upper(), 0, len(axname)) > -1:
             axnumber = ax
             break
      ...
      if n > 2: 
         ...
         for ax in range(n,0,-1):     # e.g. ax = 3,2,1
            if (ax == axnr1 or ax == axnr2):
               ...
               g = int(self.axisinfo[ax].outsidepix)
         ...
         if self.dat is not None:
            ...
            if axperm[0] != wcsaxperm[0]:
               ...
               self.boxdat = self.dat[tuple(sl)].reshape( (axlen1,axlen2) )
            else:
               #self.boxdat = self.dat[sl].reshape((self.axisinfo[axnr2].axlen,self.axisinfo[axnr1].axlen))
               self.boxdat = self.dat[tuple(sl)].reshape( (axlen2,axlen1) )
      else:
         self.boxdat = self.dat
   ...
   #--------------------------------------------------------------
      ...
      :param skyout:
         ...
         at :meth:`kapteyn.celestial.skymatrix`
      :type skyout:
   ...
   def reproject_to(self, reprojobj=None, pxlim_dst=None, pylim_dst=None,
      ...
      :Examples:
            ...
            * *plimlo=(55,1)*, *plimhi=(55,1)*
            ...
            ::
               ...
               # Rotfits.writetofits("m10rot.fits", overwrite=True, append=False)
            ...
            ::
               ...
               fnew.writetofits("classic.fits", overwrite=True, append=False)
      ...
      dst_offset = (int(pylim_dst[0]-1), int(pxlim_dst[0]-1))   # Note the order!
      ...
      if naxisout == 0:
         ...
         for tup in z:
            ...
            boxdat = self.dat[tuple(sl)].squeeze()
            boxdatnew = newdata[tuple(slnew)].squeeze()
   ...
   def writetofits(self, filename=None, comment=True, history=True,
                   ...
                   overwrite=False, append=False, extname=''):
   #---------------------------------------------------------------------
      """
      ...
      :param overwrite:
         ...
         With *overwrite=True* an existing file will be overwritten.
   ...
   #---------------------------------------------------------------------
      ...
      if append:
         if not extname:
            extname = "KP"+ datetime.now().strftime("%Y%m%d%Hh%Mm%Ss")
         if pyfitsoldversion:
         ...
         hdulist.writeto(filename, overwrite=overwrite, output_verify='silentfix')
         hdulist.close()
      # Turn warnings on
      #warnings.resetwarnings()
...
The objects are appended to a list with method :meth:`MovieContainer.append`.
...
#--------------------------------------------------------------------
   ...
   record_bitrate = 1800  # VOG: 28-06-2023, not used anymore.
   ...
   recdefs = dict(outfile=record_outfile, bitrate=record_bitrate,
                  ...
                  fps=record_fps_in, fpso=record_fps_out)
   ...
   def controlpanel(self, event, externalkey=''):
      ...
      if event is not None:
         try:
            ...
            if key.startswith('SH'):     # Do not interfere with SHIFT+arrow to move crosshair
               return
            # Catch the arrow key. The event is generated for the entire canvas
            # and is not bound to a Matplotlib Axes object. To prevent interfering
            # with the crosshair events with the SHIFT arrow keys, we check first
            # whether the event starts with SH (note that only 'S' is allowed)
            # as in SHIFT+DOWN
   ...
   def set_recorddefaults(self, outfile=None, fps=None, fpso=None, bitrate=None,
                          removetmpfiles=None):
      #---------------------------------------------------------------------------
      """
      ...
      #---------------------------------------------------------------------------
      if fps is None:
         self.record_fps_in = MovieContainer.record_fps_in
      else:
         mes = ""
         try:
            fps = int(fps)
            ...
            if not mes:   
               mes = "Invalid expression for frames per second for the input frames"
            raise Exception(mes)

      if fpso is None:
         self.record_fps_out = MovieContainer.record_fps_out
      else:
         mes = ""
         try:
            fpso = int(fpso)
            if fpso < 0:
               ...
               self.record_fps_out = fpso
         except:
            if not mes:   
               mes = "Invalid expression for frames per second for the output frames"
            raise Exception(mes)


      self.record_frameformat = MovieContainer.record_frameformat

      # bitrate is not used after 28-06-2024 update
      if bitrate is None:         
   ...
   def create_recording(self):
      ...
      #args = [cmd, '-vframes', str(self.record_counter), '-i', basename,
      #        '-framerate', str(self.record_fps)]
      args = [cmd, '-framerate', str(self.record_fps_in), '-i', basename,  '-r', str(self.record_fps_out)]
      # args = [cmd, '-framerate', str(5), '-i', basename, '-r', str(self.record_fps)]


      # Not used anymore after 28-06-2024
      #if self.record_bitrate > 0:
      #   args.extend(['-r', '%d' % self.record_bitrate])
...
class Cubeatts(object):
   #----------------------------------------------------------------------------
   """
   Class that sets cube attributes. The constructor is called by the :meth:`Cubes.append`
   ...
   :Attributes:
    ...
    .. attribute:: slicepos
       ...
       Attribute ``slicepos`` is documented in :meth:`Cubes.append`.
    ...
    .. attribute:: clipmode
       ...
       The modes are documented in method :meth:`Cubes.append`.
   ...
   def __init__(self, frame, fitsobj, axnums, slicepos=[], pxlim=None, pylim=None,
      ...
      self.panelxline2 = None
      self.panelyline  = None
      self.panelyline2 = None
...
:param callbackslist:
   ...
   are specific for a cube, see also :meth:`Cubes.append`. 
   ...
   *  'finished' -- All images are loaded and mouse- and keyboard interaction with
      the canvas is enabled. A similar callback is found in method :meth:`Cubes.append`         
...
:Example: Example 1 (Use callback functions in the constructor of a Cubes object)
   ...
   ::   
      ...
      fitsobject = maputils.FITSimage('ngc6946.fits')
      myCubes.append(frame, fitsobject, axnums=(1,2), slicepos=list(range(1,101)))
...
:Example: Use two different cube containers connected to two different figures
   ...
   ::
      ...
      def postLoading(cubecontainer, splitfr, slicepos):
         ...
         cubecontainer.set_panelframes(list(range(len(slicepos))), panel='XY')
      ...
      myCubes.append(frame2, fitsobject2, axnums=(1,3), slicepos=list(range(1,101)),
...
:Attributes:
    ...
    .. attribute:: cubelist
       ...
       Python list with cubes appended to this list by method :meth:`Cubes.append`.
    ...
    .. attribute:: splitmovieframe
       ...
       set with :meth:`Cubes.set_splitimagenr`.
...
.. automethod:: zprofileLimits
   ...
   def plotZprofile(self, cube, x, y, cubenr=None, pos=None):
      ...
      # Build the Z-profile
      yyy = []
      xi = int(numpy.round(x - (cube.pxlim[0]-1)))
      yi = int(numpy.round(y - (cube.pylim[0]-1)))
   ...
   def zprofileLimits(self, cubenr, zmin=None, zmax=None):
      ...
      :Notes:
         See example at :meth:`Cubes.set_zprofile`
      """
      #-----------------------------------------------------------------
      cube = self.cubelist[cubenr]
   ...
   def drawCrosshair(self, framenr, cubechanged=False, oldcubenr=-1):
      ...
      if cubechanged:
         ...
         if oldcube.panelXframes:
            ...
            oldcube.panelxline2.set_visible(False)
            oldcube.framep1.draw_artist(oldcube.panelxline2)
         if oldcube.panelYframes:
            oldcube.panelyline.set_visible(False)
            oldcube.framep2.draw_artist(oldcube.panelyline)
            oldcube.panelyline2.set_visible(False)
            oldcube.framep2.draw_artist(oldcube.panelyline2)
      ...
      if cube.panelXframes: 
         ...
         try:
            ...
            cube.panelxline2.set_xdata((x,x))
            #flushprint("crosshair in xpanel at index i=%d"%i)
            cube.panelxline2.set_visible(True)
            cube.framep1.draw_artist(cube.panelxline2)
      ...
      if cube.panelYframes:
         ...
         try:
            ...
            cube.panelyline2.set_ydata((y,y))
            #flushprint("crosshair in ypanel at index i=%d"%i)
            cube.panelyline2.set_visible(True)
            cube.framep2.draw_artist(cube.panelyline2)
   ...
   def append(self, frame, fitsobj, axnums, slicepos=[],
      ...
      :param axnums:
         ...
         .. only:: html
            ...
            If we want RA, FREQ images for all DEC slices, we should write:: 
               ...
               slicepos = list(range(1, n2+1))  # all DEC slices
      ...
      :param slicepos:
         ...
         .. only:: html
            ...
            Slice positions can be given in any order::
               ...
               slicepos = list(range(naxis3, 0, -1))
      ...
      :param pylim:
         ...
         .. only:: html
            ...
            Example::
               ...
               cube = myCubes.append(frame1, fitsobject, axnum(1,2), slicepos=list(range(1, n3+1)),
      ...
      :param callbackslist:
            ...
            to one cube.
            To handle these events, one must connect it to a function
            or an object with methods. With method :meth:`Cubes.append`
      ...
      :Examples:
         ...
         .. code-block:: python
            ...
            def postLoading(splitfr, slicepos):
               ...
               myCubes.set_panelframes(list(range(len(slicepos))), panel='XY')
   ...
   def set_aspectratio(self, cube, aspectratio):
      ...
      :notes:
         Note for programmers: We need a call to method :meth:`Cubes.reposition` to adjust
   ...
   def create_panel(self, panid, cube):
      ...
      if panx and cube.panelXframes:
         ...
         mplimp1.regcbkey = AxesCallback(self.eventfrompanel1, framep1,
                                           'key_press_event',
                                            cubenr=cubeindx, posobj=posobj1)
         mplimp1.AxesCallback_ids.append(mplimp1.regcbkey)
         ...
         cube.panelxline2 = framep1.axvline(ymin=ly-1-delta, ymax=0, visible=False, **lineprops)
      ...
      if pany and cube.panelYframes:
         ...
         mplimp2.regcbkey = AxesCallback(self.eventfrompanel2, framep2,
                                        'key_press_event',
                                         cubenr=cubeindx, posobj=posobj2)
         mplimp2.AxesCallback_ids.append(mplimp2.regcbkey)
      ...
      # Properties
         ...
         cube.panelyline2 = framep2.axhline(xmin=0, xmax=lx-1-delta, visible=False, **lineprops)
   ...
   def updatemovieframe(self, cb, frompanel1=False, frompanel2=False):
      ...
      subsetnr = currentindx
      #s = "Currentindx=%d"%currentindx
      #flushprint(s)
      yi = int(numpy.round(cby))
      
      if cb.event.name == 'key_press_event':
         k = cb.event.key.upper()
         #flushprint(k)
         #s = "xold=%f"%(currentcube.yold)
         #flushprint(s)
         skip = len("SHIFT+")
         arrow = k[skip:]
         if frompanel1:
            if 'UP' in arrow:
                  cbx = currentcube.xold
                  yi = subsetnr - 1
                  cby = yi
            elif 'DOWN' in arrow:
                  cbx = currentcube.xold
   ...
   def eventfrompanel1(self, cb):
      ...
      if cb.event.name == 'motion_notify_event' and cb.event.button == 1:
         self.updatemovieframe(cb, frompanel1=True)
      if cb.event.name == 'key_press_event' and cb.event.key.upper().startswith('SHIFT+'):
         self.updatemovieframe(cb, frompanel1=True)
   ...
   #----------------------------------------------------------------------------
      ...
      if self.crosshair:
         ...
         if cube.panelXframes:
            ...
            cube.framep1.draw_artist(cube.panelxline)
            cube.panelxline2.set_visible(False)
            cube.framep1.draw_artist(cube.panelxline2)
         if cube.panelYframes:
            cube.panelyline.set_visible(False)
            cube.framep2.draw_artist(cube.panelyline)
            cube.panelyline2.set_visible(False)
            cube.framep2.draw_artist(cube.panelyline2)
   ...
   #----------------------------------------------------------------------------
      ...
      method :meth:`Cubes.zprofileLimits()`.
   ...
   #----------------------------------------------------------------------------
      ...
      # These dummy print functions prove that both are the same Axes object
      # print("Frame 1=", id(cube.colorbar.cb.ax))
      # print("Frame 2=", id(cube.cbframe))

      cube.cbframe.text(0.2, 0.98, dataunits,
                        fontsize=7, fontweight='bold', color='w', rotation=90,
                        horizontalalignment='left', verticalalignment='top',
                        transform=cube.cbframe.transAxes,
                        bbox=dict(edgecolor='w', facecolor='red', alpha=0.5))
   ...
   def set_graticule_on(self, cube, mode):
    #----------------------------------------------------------------------------
      ...
      *hasgraticule* parameter in :meth:`Cubes.append` has not been set.
   ...
   #----------------------------------------------------------------------------   
      ...
      if not mode:
         ...
         if cube.grat:
            ...
            """VOG. After MPL 3.5, one cannot set these attributes directly. Replaced by function below.
            cube.grat.frame.lines = []
            cube.grat.frame2.lines = []
            cube.grat.frame.texts = []
            cube.grat.frame2.texts = []
            """
            removeLinesandTextfromframe(cube.grat.frame)
            removeLinesandTextfromframe(cube.grat.frame2)
            cube.linepieces = []
            cube.textlabels  = []
   ...
   #----------------------------------------------------------------------------
      ...
      the syntax of the system set by :meth:`Cubes.set_skyout` and :meth:`Cubes.set_spectrans`.
   ...
   def splitscreen(self, cb):
      """
      #-----------------------------------------------------------------------
      With Ctrl on the keyboard and the mouse buttons 1, 2 or 3 while moving the mouse,
      ...
      # 29-06-2023, VOG: the currentimage.image.im is a MPL AxeImage object. 
      # It was created with the plot method of the Image class. Its data 
      # was linked to currentimage.data. But after MPL 3.5 one cannot change
      # the data in currentimage.data and assume the image data has been changed
      # then also. This implies that the image data needs to be changed with a setter.
      # see alos: https://matplotlib.org/stable/api/image_api.html
      currentimage.image.im.set(data = currentimage.data)
   ...
   def updatepanels(self, cb, frompanel1=False, frompanel2=False,
      ...
      # or one uses the arrow keys together with <Shift>
      ...
      if frompanel2:
         skipupdatep2 = True
      frommain = not (frompanel1 or frompanel2)
         
      skipupdatep1 = skipupdatep2 = False  # VOG experiment

      if cb is None:
         x = xpos; y = ypos
         #s = "Entered from updatemovieframe. xpos=%f, ypos=%f"%(xpos,ypos)
         #flushprint(s)
         cubeindx = cubenr
         if x is None or y is None: # !!!!!!!! Testen
            return
      else:         
         # Note that cubenr, xdata and ydata are attributes of AxesCallback
         # while other properties are in cb.event (e.g. cb.event.name)
         # Note also that we process both motion_notify_event as key_press_event
         if cb and cb.event.name == 'motion_notify_event':
            if cb.event.button != 1:
               return    # Because to get panel interaction, we need to press the left button
         if cb and cb.event.name == 'key_press_event':
            if not cb.event.key.upper().startswith('SHIFT+'):
               return    # React only to arrow keys when shift is pressed also
      ...
      # For the arrow keys we set positions relative to the previous position
      # and not relative to the cursor position.
      if cb and cb.event.name == 'key_press_event':
            k = cb.event.key.upper()
            skip = len("SHIFT+")
            arrow = k[skip:]
      ...
      if skipupdatep1 and x == xold and not force:
         # Skip this position
         return
      ...
      if self.crosshair:
         ...
         if cube is currentcube:
               #flushprint("Cube is current cube")
      ...
      # PANEL 1 = PANEL X = Horizontal panel
      # If the crosshair position is changed in the main panel, 
      # we need perhaps a new image in the slice panels. Note that
      # yi and yold are integer positions and must be different then.
      # The event source cannot be panel X because cursor changes in 
      # that panel in y only changes the subset in main and 
      # changes in x do not change the slice because the slice is taken along
      # the x axis.
      
      # New image data for panel X
      if cube.panelXframes and ((yi != yold and not frompanel1) or force):
      ...
      if cube.panelXframes and (((y != yold or x != xold)) or force): 
         framep1.draw_artist(mplimp1.image.im)
         if self.crosshair and cube is currentcube:
            if frompanel1:
               cube.panelxline.set_ydata((y,y))
            framep1.draw_artist(cube.panelxline)
            if not frompanel2:
               cube.panelxline2.set_xdata((x,x))
            framep1.draw_artist(cube.panelxline2)
      ...
      # PANEL 2 = PANEL Y = VERTICAL PANEL
      # Second frame which shares an y axis with main plot
      if cube.panelYframes and ((xi != xold and not frompanel2) or force):
         M = cube.My
         lx = M.shape[1]
         j = 0
         for indx in cube.panelYframes:
      ...
      if cube.panelYframes and (((x != xold or y != yold)) or force):   
         framep2.draw_artist(mplimp2.image.im)
         if self.crosshair and cube is currentcube:
            if frompanel2:
                cube.panelyline.set_xdata((x,x)) 
            framep2.draw_artist(cube.panelyline)
            if not frompanel1:
               cube.panelyline2.set_ydata((y,y))         
            framep2.draw_artist(cube.panelyline2)
      ...
      backgroundchanged = False
      if frompanel1:   # Event from panel2: Only x has been changed
         cube.xold = xi
      elif frompanel2: # Event from panel1: Only y has been changed
         cube.yold = yi
   ...
   def loadimages(self, cb):
      ...
      mplim = cube.fitsobj.Annotatedimage(cube.frame, cmap=cube.cmap,
                                          ...
                                          gridmode=cube.gridmode, adjustable='box', anchor='NW',
      ...
      for cube in self.cubelist:
         if cube.nummovieframes >= 1:
            cube.panelscb = AxesCallback(self.updatepanels,
                                        ...
                                        'motion_notify_event',
                                         cubenr=cube.cnr)
            cube.panelscbkey = AxesCallback(self.updatepanels,
                                            cube.frame,
                                           'key_press_event',
                                            cubenr=cube.cnr)
   ...
   def end_interactionMovieToPanels(self):
      ...
      for cube in self.cubelist:
         ...
         if cube.panelscbkey is not None:
            cube.panelscbkey.deschedule()            
