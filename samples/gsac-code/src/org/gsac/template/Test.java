 ...
 * Tester
 */
public class @MACRO.PREFIX@Test implements Runnable {
    ...
    public @MACRO.PREFIX@Test(String server) {
        this.server = server;
    }
        ...
        for(int i=0;i<1000;i++) {
            for(String url: urls) {
                try {
                    ...
                    IOUtil.readContents(url, @MACRO.PREFIX@Test.class);
                } catch(Exception exc) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i=0;i<NUM_THREADS;i++) {
            executor.submit(new @MACRO.PREFIX@Test(server));
        }
