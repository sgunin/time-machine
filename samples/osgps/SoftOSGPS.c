...
{
  double acq_thres_db = 34;  /* dB-Hz, this a generally accepted value */
...
main (int argc UNUSED, char *argv[] UNUSED)
{
  ...
  while ((opt = getopt(argc, argv, "3pf:t:u")) != -1) {
    ...
    case 'u':
      ...
      break;
    default: /* '?' */
      fprintf (stderr, "Usage: %s [-3] [-p] [-t TCP_port] [-u] [-f filename]\n",
	       argv[0]);
      exit (EXIT_FAILURE);
    }
  }
  ...
  if (socket_flag) {
    ...
    utctime=buffer.st_mtime ; /* for this data set  */
  }
          ...
          for (ch = 0; ch < N_channels; ch++)
            {
              if (ichan[ch].state == off)
                {
                  long carrier_corr = -clock_offset * 1575.42 / Carrier_DCO_Delta;
		          setup_channel (ch, cold_prn, 0, dop_sign*carrier_corr); /* opposite sign of GP2021 */
                  cold_prn = cold_prn % 32 + 1;
                  break;
                }
            }
      ...
      if (xyz[prnn].elevation > mask_angle && gps_alm[prnn].health == 0 &&
          gps_alm[prnn].ety != 0.00)
        {
	      ...
	      for (ch = 0; ch < N_channels; ch++)
		      ...
		      setup_channel (ch, prnn, code_corr, dop_sign*carrier_corr);  /* opposite sign of GP2021  */
                      break;
                    }
                }
            }
